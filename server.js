...
// Removed duplicate Firebase init block
...
EXTRACTION RULES — be granular, but STRICTLY FAITHFUL to the source:
- FIDELITY (the most important rule): Extract only actions, topics, populations, and timepoints that the source text EXPLICITLY states. Granularity means faithfully separating distinct items the guideline actually writes down — it does NOT mean inventing extra rules by expanding a general instruction into specific sub-types, methods, examples, or timepoints the source never names. Before splitting, confirm the distinction actually appears in the text; if it does not, keep it as a single rule. When the guideline gives a general action followed by 'including', 'such as', or 'e.g.', the examples illustrate the single action — they are NOT each a separate rule. A general action stated once is ONE rule, however many ways it could hypothetically be carried out. In particular, a general instruction to ensure appropriate staff, equipment, facilities, or resources are available or ready is ONE rule — do NOT expand it into a separate rule for each individual person, role, specialty, device, or resource the guideline does not itself name.
...
- Each row of a care SCHEDULE or timed-action table is its OWN rule: the gestation/timepoint is the condition and each listed item is the action. E.g. "at 28 weeks: take HbA1c" → one rule (condition 'gestational age is 28 weeks', action 'take an HbA1c'); "at 36 weeks: discuss birth plan; offer colostrum expression; offer membrane sweep" → THREE separate rules. Itemise every action the schedule ACTUALLY lists, and NEVER collapse a listed schedule into a vague statement like 'monitor regularly'. But do NOT fabricate timepoints or actions the schedule does not contain — if the guideline says 'serial growth scans from 28 weeks' without listing each individual week, that is ONE rule, not one rule per week.
- A brief one-line offer, advice, or referral is still a rule even when stated tersely (e.g. 'offer anaesthetic assessment in the third trimester', 'advise antenatal colostrum harvesting from 36 weeks', 'refer to a dietitian'). Extract it as its own rule.
- Split genuinely-distinct clinical decisions the source states separately — but when you are unsure whether the text actually distinguishes two items, keep them as ONE rule rather than inventing a distinction the source does not make. Fidelity beats granularity.
...
- DO NOT extract general management philosophies or protocol references (e.g. 'follow ABCD approach', 'use multidisciplinary team', 'adhere to local protocols') — only extract the specific individual actions within them. This exclusion applies ONLY to vague directives that name no specific action; a specific scheduled, timed, or named action (e.g. 'take HbA1c at 28 weeks', 'offer anaesthetic review', 'offer colostrum expression from 36 weeks') is ALWAYS a rule, never a 'protocol reference'.
...
${body}`;
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
                try {
                    sectionResult = await routeToAI({
                        messages: [
                            {
                                role: 'system',
                                content: 'You are a clinical guideline auditor. Return ONLY a valid JSON array of objects. No preamble, no markdown, no conversational text. Be EXHAUSTIVE — extract every atomic practice rule from the specified section. Include counselling requirements, consent items, post-procedure care advice, test interpretation rules, risk communication points, clinical decision rules, scheduled/timed actions (EVERY gestation-specific action in a care schedule, as its own rule), and brief one-line offers/advice/referrals. Extract every atomic rule the text actually states, and never summarise a written-out schedule or list into a general statement — itemise what is there. But every rule must be grounded in the source: do NOT invent extra rules to be more granular by expanding a general action into specific sub-types, methods, or timepoints the text does not name — nor into a separate rule for each individual person, role, specialty, or resource implied by a general "ensure appropriate staff/equipment are available" instruction. Each distinct counselling topic, risk factor, scheduled action, or clinical action that the source explicitly states should be a separate rule.'
                            },
                            {
                                role: 'user',
            ...
            if (!sectionResult) {
                log('extract_section_done', `Section ${i + 1}/${sections.length} "${section}": FAILED after retries (skipped)`, { section, count: 0, runningTotal: allPoints.length, failed: true });
                if (i < sections.length - 1) await new Promise(resolve => setTimeout(resolve, 200));
                continue;
            }
...
app.post('/_deprecated_seedHospitalTrustMappings', authenticateUser, async (req, res) => {
    try {
        ...
        const existingSnapshot = await db.collection('hospitalTrustMappings').get();
        if (!existingSnapshot.empty) {
            console.log(`[SEED_TRUST_MAPPINGS] Clearing ${existingSnapshot.size} existing mappings...`);
        ...
        hospitalTrustMappingsCache = null;
        hospitalTrustMappingsCacheTimestamp = 0;

        console.log(`[SEED_TRUST_MAPPINGS] Successfully seeded ${totalCount} mappings`);

        res.json({
            success: true,
            message: `Successfully seeded ${totalCount} hospital trust mappings`,
            count: totalCount
        });
    } catch (error) {
...
Work in two passes:
1. PLACE the new information where that KIND of content belongs (see the section guide below). Decide placement by what the content is and when it applies, not by which topic it sits near — and never file something under a heading whose timing or meaning contradicts it. If the note already documents it, refine the existing line in place rather than appending a duplicate.
2. RECONCILE the note so it reads as a single coherent entry, not an accumulation. Reading the whole note:
   - If the new information contradicts an existing statement, or makes it inaccurate or misleading, fix or remove that statement.
   - If the new information repeats or overlaps something already documented — in whole or in part — merge it into the existing entry rather than leaving two copies. State each action, plan, or counselling point once.
   - If related content already sits in the wrong section, move it to where it belongs.

Be surgical. Make the fewest, smallest edits that achieve the above: fix only the statements that genuinely conflict or duplicate, and leave every correct, non-overlapping line exactly as written, in its place. Do not restructure sections, renumber items, or rewrite lines for style. Your output is diffed against the original and shown to the user for review before anything is applied, so unnecessary edits are just review noise.
