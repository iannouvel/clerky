...
async function callGemini(systemPrompt, userPrompt, userId = null, meta = null, taskComplexity = 'simple', preferredProvider = null, forceDirect = false) {
    ...
    try {
        ...
        if (!forceDirect && typeof routeToAI === 'function') {
            ...
            const r = await routeToAI({ messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }], temperature: 0 }, userId, preferredProvider, 4000, taskComplexity);
...
- When a practice point concerns the patient SELF-MONITORING a measurement against a target or target range over time (rather than a single clinic measurement), capture ONE value for how well controlled they are against that target — whether their self-monitored readings are generally within range and how consistently — NOT a separate spot value for each monitored parameter or each target threshold. Self-monitoring describes an ongoing pattern, so the meaningful value is the in-range control status, never a single one-off reading.
...
Draw the line carefully between the ongoing management of an existing condition (which APPLIES) and a discrete later step or stage of care that has not yet been reached (which does not apply yet). A future stage the note merely plans, books or anticipates — labour, birth, a caesarean, the postnatal period — is not the patient's current stage: values that record what is done during or after that stage only become answerable once it is genuinely underway or imminent, so treat them as not_applicable at an encounter that precedes it, even when the plan clearly intends it. Keep the decision- and preparation-stage values that are live now. When genuinely unsure whether a step is yet due, choose "applies" — but do NOT choose "applies" for a point written for a different population than the patient's, or for the conduct of a stage the note shows only as a future plan.
