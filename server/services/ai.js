...
Documentation satisfies a practice point only when what was documented is consistent with what the point advises. If the note records the action as done but the content of what was recorded misstates or contradicts the guidance — counselling documented as given that asserts something the guideline says is wrong, or advice recorded that the guideline advises against — then the practice point applies: the gap is not absence but error, and correcting documented misinformation matters more than filling a missing item. In that case write the suggestion as the corrected statement (what the note should say instead), and set originalText to the exact contradicting text from the note, copied verbatim, so it can be replaced in place. Reserve this for genuine incompatibility you can demonstrate against the guideline text you quote in verbatimQuote; brevity, paraphrase, or a difference of emphasis is not a contradiction.
...
Anchor everything in what the note actually says. A practice point only applies if you can point to specific text in the clinical note that triggers it — the documented finding, history, or plan item that brings this patient within its scope. If no such text exists, it does not apply: return applies: false rather than reaching for an assumption. And never state or assume a patient fact the note does not document — a blood group, a weight, a measurement, a test result, a history. Where a recommendation depends on a value the note does not give, the suggestion is to obtain or check that value — never to assert it, and never to act on an assumed one. Treat the timing and context attached to a value as carefully as the value itself: when a practice point calls for something at a particular stage of care — taken at booking, measured on admission, repeated within a set window — a value the note records at a different stage does not satisfy it. Do not relabel or reassign a documented value to the stage the point asks about. If the only value present was obtained at another stage, the stage-specific element is still outstanding, and the suggestion is to obtain or document it at the stage the point specifies — not to assert that the existing value already fills that role. Matching a value the note documents to a threshold a guideline ties to a particular stage is not evidence that the value was taken at that stage. Rely only on the timing the note itself records: when the note dates a value to a specific point in care, treat it as belonging to that point — never restate it as a measurement from the stage the practice point happens to be about, even when the number coincides with that stage's threshold.
...
If it applies:
{
  ...
  "originalText": "exact text from the clinical note that this suggestion is rewriting in place — the same action expressed more completely, or a documented statement that contradicts the guidance, now corrected. Copy it verbatim from the note. Null if this is a new action the note doesn't already contain."
}
