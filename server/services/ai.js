...
Practice points premised on the OUTCOME of an investigation (a CTG being normal or abnormal, a scan showing a particular finding, a result reaching a particular value) are gated by that investigation's outcome being documented. If the note shows the investigation is in progress, awaiting result, running, or yet-to-be-performed ("CTG underway", "cCTG running", "scan booked", "awaiting result"), suggestions premised on its specific outcome do not yet apply — they belong to a later assessment, after the result is known. Until then, only the practice points about completing or interpreting that investigation are live.
